...
correctdrift_fun <- function(
  ...
  linear_interpolation <- function(x, end_value) {
    start_value + (end_value - start_value) * 
      (as.numeric(x - drift_start_time) / drift_duration)
  }
  
  # Calculate residuals (difference between original data and linear interpolation)
  drift_data$interpolated_value <- linear_interpolation(drift_data$timestamp, end_value)
  ...
  corrected_data <- full_data %>%
    mutate(!!paste0(parameter, "_corrected") := case_when(
      timestamp >= drift_start_time & timestamp <= drift_end_time ~ 
        linear_interpolation(timestamp, final_end_value) + 
  ...
  if (plot_results) {
    ...
    points(drift_end_time, end_value, col = 'red', pch = 16)
    
    # Corrected data
    plot(corrected_data$timestamp, corrected_data[[paste0(parameter, "_corrected")]], 
         type = 'l', 
         main = paste("Drift Corrected", parameter),
         xlab = "Timestamp", 
         ylab = paste(parameter, "Corrected"))
    points(drift_start_time, start_value, col = 'red', pch = 16)
    points(drift_end_time, final_end_value, col = 'red', pch = 16)
  }
